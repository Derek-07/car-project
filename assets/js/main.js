(function ($) {
  ...
  /*---------- 01. On Load Function ----------*/
    ...
    $(".advance-search-btn").click(function(event){
...
/* ===== 3D Porsche control only on hover ===== */
/* ===== Porsche auto-rotate + click-to-control ===== */

$(document).ready(function () {
    const model = document.getElementById("porscheModel");
    if (!model) return;
    ...
    // Enable control ONLY when user clicks / drags
    model.addEventListener("pointerdown", function () {
        model.cameraControls = true;
    });

    // Disable control when interaction ends
    model.addEventListener("pointerup", function () {
        model.cameraControls = false;
    });
...
const swiper = document.querySelector('#heroSlidee1').swiper;

swiper.on('slideChange', function () {

    document.querySelectorAll('model-viewer').forEach(function (model, index) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!video || !muteBtn) {
        console.log("Video or mute button not found");
        return;
    }
    
    const icon = muteBtn.querySelector("i");

    // Initialize: unmuted with sound
    video.muted = false;
    video.volume = 0.5;
